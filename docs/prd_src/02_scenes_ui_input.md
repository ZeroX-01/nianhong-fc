...
*   **13 个交互热区**（逻辑坐标 x, y, w, h，来自 `SB.ROOM` + `buildSpots()`）：
    ...
    | **小方桌 (desk)** | (390, 213, 66, 49) | 写作业（进 `HomeworkScene`）/ 帮家里干活（`sub` 写着这会儿有几样能干）/ 把作业本摊开 |
...
*   **关键机制**：
    ...
    - 出门前电视还开着会被拦住。
    - **`choreMenu()` / `doChore(id)`**：小方桌那一屏先摆一行灰的状态（妈在厨房 / 妈下地去了），底下四样活（刷碗、扫地、倒垃圾、给小卖部搬货）。**干不了的活不置灰**——灰行光标跳不过去，玩家就读不到「得等下一顿饭吃完」这种原因，所以照样能选，选中了讲一句为什么。规则全交给 `SB.Chore`，客厅只管演。
    - **`settleOwed()`**：进客厅先看 `Chore.owed()`；妈不在家时干的活钱是挂着的，她一回屋就补一屏收钱特写再进兜。
    - **`alive()`**：收钱特写的 `onDone` 可能在客厅已经 shutdown 之后才回来，账必须照记，但刷 HUD / 飘字得先问一句 `alive()`，否则会往销毁掉的控件上写字。
...
### 5.4.1 收钱特写（PayAnim，不是场景）
*   **职责**：干完活 / 卖完瓶子之后，给钱那一下要有一屏，看着像吹卡带、划桌子那种独立画面，但实现上是 `src/anim/payAnim.js` 的**全屏覆盖层**，不做真的场景切换（切场景会打断客厅的妈妈秒表与热区状态）。
*   **画面结构**：横向居中的取景框（`200×140` 逻辑像素）里演五拍 —— 招呼 / 伸手掏 / 掏出来 / 递过来 / 松手；框底下是整条字幕带（高 57），前三行走台词，第四行固定是「按 A 一张一张数」。台词最长折三行，压不到提示那一行。
*   **付款方按来源分**（`SB.PAY_BY_SRC` → `SB.PAYERS`）：家里的活和期末成绩单是**妈**（厨房门口）；瓶子、废纸、旧课本是**收废品的老汉**（院门外）；跑腿搬货是**小卖部老板**（柜台）。沙发缝里摸出来的钱没人给，不起这一屏。
*   **数钱交互**：按 A 一枚一枚数（面额由 `Chore.breakdown()` 拆，最多 8 件），按 B / START 一把数完，**两种走法金额完全一样，跳过不罚钱**。
*   **看第几遍就演多快**：第一遍五拍全演（约 2860ms），第二遍整体压到约 1973ms，第三遍起只留掏钱那三拍（约 820ms）。开了系统的 `prefers-reduced-motion` 直接给静帧。
*   **钱的唯一出口**：`onDone(amount)` 保证**只回调一次**——按 A 数完、按 B 跳过、甚至整个客厅被 `scene.stop()` 掐掉，都只结一次账。主动家务的钱在 `onDone` 里通过 `SB.Story.earn()` 进兜；日常随机事件的钱 `SB.Time.rollEvent()` 已经记过，那一屏纯粹是演给玩家看，不许再加。
*   **输入互锁**：演出期间 `SB.__interludeOpen++`，客厅热区和菜单全锁，收起时减回去；异常路径也得减，不然会卡成点不动。
